...
struct SearchMembersView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                NavigationLink {
                    NewVisitorView(viewModel: .init(viewModel.event, storage: viewModel.storage)) {
                        dismiss()
                    }
                } label: {
                ...
                ForEach(viewModel.members) { member in
                    NavigationLink {
                        VisitorView(viewModel: .init(member: member, event: viewModel.event, storage: viewModel.storage)) {
                            dismiss()
                        }
                    } label: {
